...
The release workflow is intentionally split into platform jobs and a publish job. `workflow_dispatch` is a rehearsal path; the tag-triggered path is the only publication path. RC3 must reach green native CI and a successful rehearsal before a release tag is created.
