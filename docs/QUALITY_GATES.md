...
On Linux CI only, the smoke process receives Chromium's `--no-sandbox` flag because hosted runners do not provide the setuid sandbox ownership required by Electron. This is a test-runner accommodation; packaged runtime hardening and fuse assertions remain unchanged.
