...
import { assertReleaseCompleteness, loadReleaseMatrix, parseChecksumManifest, SBOM_NAME } from './release-contracts.mjs';
...
for (const platform of ['macos', 'windows', 'linux']) {
  ...
  const evidenceNames = new Set(readdirSync(assetDir).filter((name) => name.startsWith('RELEASE-EVIDENCE-') && name.endsWith('.json')));
