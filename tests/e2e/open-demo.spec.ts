...
test('Open Demo Project action opens the bundled ForgeShop project end-to-end', async () => {
  // Source-mode launch: no FORGELOOP_AUDIT_FIXTURE_PROJECT injection, so the
  // project can only appear through the real Project Picker → preload → IPC →
  // resolveBundledDemoPath → openProject pipeline.
  const app = await electron.launch({
    args: ['.'],
    env: { ...process.env, NODE_ENV: 'production', FORGELOOP_AUDIT_SMOKE: '1' },
  });
  try {
    const window = await app.firstWindow();

    await expect(window.getByRole('heading', { name: 'ForgeLoopAudit' })).toBeVisible();
    ...
    await expect(window.getByRole('heading', { name: 'Audit Summary' })).toBeVisible({ timeout: 15_000 });
    ...
    await window.getByLabel('Main navigation').getByRole('button', { name: 'Policy & Trust', exact: true }).click();
    await expect(window.locator('h1').filter({ hasText: 'Policy & Trust' })).toBeVisible({ timeout: 5000 });

    // The visible ForgeLoopAudit version must come from the runtime, matching package.json.
    await window.getByLabel('Main navigation').getByRole('button', { name: 'Settings', exact: true }).click();
    await expect(window.locator('body')).toContainText(`ForgeLoopAudit v${packageVersion}`);
  } finally {
    await app.close();
  }
});
